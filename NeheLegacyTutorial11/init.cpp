...
#define POINT_COORD_COUNT	3
#define GRID_COUNT			45
...
struct point_t
{
	union
	{
		float raw_data[POINT_COORD_COUNT];
		struct
		{
			float x, y, z;
		};
...
point_t points[GRID_COUNT][GRID_COUNT];
...
int init_gl_texture()
{
	image::tga_image texture01("textures/win_logo.tga");
...
int draw_gl_scene(GLvoid)
{
	...
	for (size_t x = 0; x < GRID_COUNT - 1; x++)
	{
		...
		{
			f_x = float(x) / (GRID_COUNT - 1);
			f_y = float(y) / (GRID_COUNT - 1);
			f_xb = float(x + 1) / (GRID_COUNT - 1);
			f_yb = float(y + 1) / (GRID_COUNT - 1);
...
void init_points()
{
	for (size_t x = 0; x < GRID_COUNT; x++)
	{
		for (size_t y = 0; y < GRID_COUNT; y++)
		{
			...
			points[x][y].z = float(sin((x / 5.0f) * 40.0f * M_PI / 180.0f));
		}
	}
}
