...
#define M_PI_180 (float)M_PI / 180.0f
...
sector_t *p_sector = nullptr;

GLfloat y_rot = 0.0f;
GLfloat heading = 1.0f;
GLfloat x_pos = 0.0f;
GLfloat z_pos = 0.0f;

GLfloat walk_bias = 0.0f;
GLfloat walk_bias_angle = 0.0f;
GLfloat lookup_down = 0.0f;
GLfloat z_pan = 0.0f;
...
int init_gl_texture()
{
	image::tga_image texture01("textures/Mud.tga");
...
int draw_gl_scene(GLvoid)
{
	...
	GLfloat x_trans = -x_pos;
	GLfloat z_trans = -z_pos;
	GLfloat y_trans = -walk_bias - 0.25f;
	GLfloat scene_y_rot = 360.0f - y_rot;

	glTranslatef(0.0f, -0.25f, 0.0f);

	glRotatef(lookup_down, 1.0f, 0.0f, 0.0f);
	glRotatef(scene_y_rot, 0.0f, 1.0f, 0.0f);
	...
	for (size_t i = 0; i < (size_t)p_sector->n_triangles; i++)
	{
		glNormal3f(0.0f, 0.0f, 1.0f);
		for (size_t j = 0; j < TRINGLE_VERT_COUNT; j++)
		{
			glTexCoord2f(p_sector->triangle[i].vertex[j].u, p_sector->triangle[i].vertex[j].v);
			glVertex3f(p_sector->triangle[i].vertex[j].x, p_sector->triangle[i].vertex[j].y, p_sector->triangle[i].vertex[j].z);
		}
	}
	glEnd();
...
{
	load_world();
	return (p_sector = get_world()) != nullptr;
}
...
BOOL create_gl_window(LPCWSTR title, int width, int height, int bits, bool is_fullscreen)
{
	...
	{
		kill_gl_window();
		MessageBox(nullptr, L"Initialization world failed", L"Create window Error", MB_OK | MB_ICONEXCLAMATION);
		return FALSE;
	}
...
bool key_handler()
{
	...
	if (keys[VK_RIGHT]) y_rot = heading -= 1.0f;
	if (keys[VK_LEFT]) y_rot = heading += 1.0f;

	if (keys[VK_UP])
	{
		x_pos -= (float)sin(heading * M_PI_180) * 0.05f;
		z_pos -= (float)cos(heading * M_PI_180) * 0.05f;

		walk_bias_angle += 10;
		if (walk_bias_angle > 360.0f) walk_bias_angle = 0.0f;

		walk_bias = (float)sin(walk_bias_angle * M_PI_180) * 0.05f;
	}
	...
	if (keys[VK_DOWN])
	{
		...
		walk_bias_angle -= 10;
		if (walk_bias_angle < 0.0f) walk_bias_angle = 0.0f;
