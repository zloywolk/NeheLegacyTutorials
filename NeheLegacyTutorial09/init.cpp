...
int init_gl_texture()
{
	image::tga_image texture01("textures/Star.tga");
...
int draw_gl_scene(GLvoid)
{
	...
	for (size_t i = 0; i < STAR_COUNT; i++)
	{
		...
		glTranslatef(stars[i].distance, 0.0f, 0.0f);
		...
		{
			glColor4ub(
				stars[STAR_COUNT - i - 1].r,
				stars[STAR_COUNT - i - 1].g,
				stars[STAR_COUNT - i - 1].b,
				255);
			glBegin(GL_QUADS);
		...
		spin += 0.01f;
		stars[i].angle += float(i) / STAR_COUNT;
...
void init_stars()
{
	...
	{
		stars[i].angle = 0.0f;
		stars[i].distance = (float)i / STAR_COUNT * 5.0f;
